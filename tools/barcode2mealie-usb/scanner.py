...
for letter in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
    NORMAL[f"KEY_{letter}"] = letter.lower()
    SHIFTED[f"KEY_{letter}"] = letter
...
def decode_key(key_name: str, shifted: bool) -> str | None:
    table = SHIFTED if shifted else NORMAL
    return table.get(key_name) or NORMAL.get(key_name)
...
def main() -> int:
    ...
    log.info("Listening on %s (%s), posting to %s", device_path, device.name, scan_url())
    ...
    for event in device.read_loop():
        ...
        if key_name in SHIFT_KEYS:
            ...
            continue
        if event.value != 1:  # key-down only; ignore key-up and repeat
            continue
        ...
        char = decode_key(str(key_name), shift_down)
