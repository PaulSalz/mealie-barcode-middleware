...
def shopping_lists(force: bool = Query(False), db: Session = Depends(get_db)):
    ...
    return {
        ...
        "items": [{**row, "default": str(row.get("id")) == str(default_id)} for row in rows],
    }
...
async def save_default_list(request: Request, db: Session = Depends(get_db)):
    if denied := _require_admin_json(request):
        return denied
    body = await request.json()
    try:
        list_id = set_default_shopping_list_id(str(body.get("list_id") or ""), db)
...
def barcode_destination(barcode: str = Query(..., min_length=1), db: Session = Depends(get_db)):
    ...
    if not targets and not mapping:
        return {"mapped": False, "targets": []}
    ...
    if targets:
        for target in targets:
            ...
            if effective_route == "default":
                effective_route = "mealie"
            ids = _target_lists(target)
            if not ids and item and item.shopping_list_id:
                ids = [item.shopping_list_id]
        ...
        result.append({
            ...
            "route": "inherit",
            "effective_route": (item.shopping_route if item and item.shopping_route not in {None, "default"} else "mealie"),
            ...
            "recipe_scale": mapping.recipe_scale,
        })
