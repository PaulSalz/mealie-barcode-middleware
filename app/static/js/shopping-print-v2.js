(function () {
  ...
  var state = {
    ...
    pollIntervalMs: 5000,
    labelTypes: [],
    overrideDraft: {key: '', dirty: false, name: '', quantity: '', unit: ''},
  };
  ...
  var els = {
    ...
    overrideUnit: $('shopping-print-override-unit'),
  ...
  function setRadio(name, value, fallback) {
    var input = document.querySelector('input[name="' + name + '"][value="' + CSS.escape(String(value || fallback)) + '"]');
    if (!input && fallback != null) input = document.querySelector('input[name="' + name + '"][value="' + CSS.escape(String(fallback)) + '"]');
    if (input) input.checked = true;
  }

  function markerStyle() {
    return selectedRadio('sp-item-marker-style', 'checkbox');
  }

  function dividerStyle() {
    return selectedRadio('sp-category-divider-style', 'solid');
  }

  function populateLabelTypes(rows) {
    var select = $('sp-label-type');
    if (!select) return;
    state.labelTypes = Array.isArray(rows) ? rows.slice() : [];
    if (!state.labelTypes.length) {
      state.labelTypes = [
        {value: 1, name: 'With gaps'},
        {value: 3, name: 'Continuous'},
      ];
    }
    select.innerHTML = state.labelTypes.map(function (row) {
      return '<option value="' + Number(row.value) + '">' + esc(row.name || ('Type ' + row.value)) + '</option>';
    }).join('');
  }
  ...
  function inputSettings() {
    ...
    return {
      ...
      category_divider_style: dividerStyle(),
    };
  }

  function syncConditionalSettings() {
    var showItems = $('sp-show-items').checked;
    var showCategoryDividers = $('sp-show-category-dividers').checked;
    document.querySelectorAll('input[name="sp-item-marker-style"]').forEach(function (input) { input.disabled = !showItems; });
    document.querySelectorAll('input[name="sp-category-divider-style"]').forEach(function (input) { input.disabled = !showCategoryDividers; });
  ...
  function populateSettings(settings) {
    ...
    $('sp-label-type').value = String(state.settings.label_type == null ? 3 : state.settings.label_type);
    ...
    setRadio('sp-category-divider-style', state.settings.category_divider_style === 'dashed' ? 'dashed' : 'solid', 'solid');
    setRadio('sp-item-marker-style', state.settings.item_marker_style || (state.settings.show_checkboxes === false ? 'none' : 'checkbox'), 'checkbox');
  ...
  function rememberOverrideDraft() {
    if (!els.overrideItem) return;
    state.overrideDraft.key = els.overrideItem.value || '';
    state.overrideDraft.name = els.overrideName ? els.overrideName.value : '';
    state.overrideDraft.quantity = els.overrideQty ? els.overrideQty.value : '';
    state.overrideDraft.unit = els.overrideUnit ? els.overrideUnit.value : '';
  }

  function markOverrideDirty() {
    rememberOverrideDraft();
    state.overrideDraft.dirty = true;
    setStatus(els.overrideStatus, 'Unsaved override changes.', 'yellow');
  }

  function syncOverrideEditor(force) {
    if (!els.overrideItem) return;
    var item = currentItemForOverride();
    var key = item ? String(item.override_key || '') : '';
    if (!force && state.overrideDraft.dirty && state.overrideDraft.key === key) {
      if (els.overrideName) els.overrideName.value = state.overrideDraft.name;
      if (els.overrideQty) els.overrideQty.value = state.overrideDraft.quantity;
      if (els.overrideUnit) els.overrideUnit.value = state.overrideDraft.unit;
      updateButtons();
      return;
    }
    var saved = item ? overrideForKey(key) : null;
    if (els.overrideName) els.overrideName.value = saved ? (saved.name_alias || '') : '';
    if (els.overrideQty) els.overrideQty.value = saved ? (saved.quantity_alias || '') : '';
    if (els.overrideUnit) els.overrideUnit.value = saved ? (saved.unit_alias || '') : '';
    state.overrideDraft = {
      key: key,
      dirty: false,
      name: els.overrideName ? els.overrideName.value : '',
      quantity: els.overrideQty ? els.overrideQty.value : '',
      unit: els.overrideUnit ? els.overrideUnit.value : '',
    };
    updateButtons();
  }
  ...
  function renderOverrideEditor() {
    ...
    var previous = els.overrideItem.value || state.overrideDraft.key || '';
  ...
  function renderReceipt() {
    if (!state.data) {
      ...
      els.paperSize.textContent = (state.settings.paper_width_mm || 50) + ' mm × —';
      return;
    }
  ...
  function dataFingerprint(data) {
    return JSON.stringify(((data && data.items) || []).map(function (item) {
      return [
        ...
        item.original_quantity_value_text || item.quantity_value_text || '',
        item.original_unit_text || item.unit_text || '',
        item.category,
        item.override_key,
      ];
  ...
  async function persistLocalContent(successText) {
    if (!state.data) throw new Error('Choose a shopping list first.');
    ...
    if (!data.ok) throw new Error(data.error || 'Could not save print-only content.');
    ...
    renderLocalEntries();
    renderReceipt();
    if (successText) setStatus(els.localStatus, successText, 'success');
    return data;
  }
  ...
  async function bootstrap() {
    ...
    try {
      ...
      state.pollIntervalMs = Math.max(1000, Number(data.poll_interval_seconds || 5) * 1000);
      populateLabelTypes(data.label_types || []);
  ...
  els.list.addEventListener('change', function () {
    lastFingerprint = '';
    state.overrideDraft = {key: '', dirty: false, name: '', quantity: '', unit: ''};
  ...
  els.saveOrder.addEventListener('click', async function () {
    ...
    try {
      ...
      if (!data.ok) throw new Error(data.error || 'Could not save route.');
      state.order = (data.category_order || state.order).slice();
      state.aliases = Object.assign({}, data.category_aliases || {});
      renderOrder();
      renderReceipt();
  ...
  if (els.localAdd) {
    els.localAdd.addEventListener('click', async function () {
      ...
      var snapshot = state.localEntries.map(function (entry) { return Object.assign({}, entry); });
      ...
      renderLocalEntries();
      renderReceipt();
      els.localAdd.disabled = true;
      setStatus(els.localStatus, 'Saving new entry…', 'secondary');
      try {
        await persistLocalContent('Entry added and saved for this list.');
        els.localName.value = '';
        els.localQty.value = '';
        els.localCategory.value = '';
        els.localName.focus();
      } catch (error) {
        state.localEntries = snapshot;
        normalizeOrder(true);
        renderOrder();
        renderLocalEntries();
        renderReceipt();
  ...
  if (els.localEntries) {
    els.localEntries.addEventListener('click', async function (event) {
      ...
      renderLocalEntries();
      renderReceipt();
      setStatus(els.localStatus, 'Removing entry…', 'secondary');
      try {
        await persistLocalContent('Entry removed and saved. Unused categories were cleaned up.');
      } catch (error) {
        state.localEntries = snapshot;
        normalizeOrder(true);
        renderOrder();
  ...
  if (els.saveLocal) {
    els.saveLocal.addEventListener('click', async function () {
      ...
      state.localComment = els.localComment ? els.localComment.value : state.localComment;
      setStatus(els.localStatus, 'Saving print-only content…', 'secondary');
      try {
        await persistLocalContent('Print-only content saved for this list.');
      } catch (error) {
        setStatus(els.localStatus, error.message, 'danger');
      } finally {
        updateButtons();
  ...
  if (els.overrideItem) {
    els.overrideItem.addEventListener('change', function () {
      state.overrideDraft = {key: els.overrideItem.value || '', dirty: false, name: '', quantity: '', unit: ''};
      syncOverrideEditor(true);
      setStatus(els.overrideStatus, '', '');
    });
  }
  [els.overrideName, els.overrideQty, els.overrideUnit].filter(Boolean).forEach(function (input) {
    input.addEventListener('input', markOverrideDirty);
  });
  ...
  if (els.overrideSave) {
    els.overrideSave.addEventListener('click', async function () {
      ...
      var selectedKey = String(item.override_key || '');
      var desiredName = String(els.overrideName.value || '').trim();
      var desiredQty = String(els.overrideQty.value || '').trim();
      var desiredUnit = String(els.overrideUnit && els.overrideUnit.value || '').trim();
      els.overrideSave.disabled = true;
      setStatus(els.overrideStatus, 'Saving override…', 'secondary');
      try {
        var data = await fetchJson('/api/shopping-print/item-overrides', {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({
            list_id: state.data.id,
            key: selectedKey,
            name_alias: desiredName,
            quantity_alias: desiredQty,
            unit_alias: desiredUnit,
        ...
        if (!data.ok) throw new Error(data.error || 'Could not save override.');
        state.overrideDraft = {key: selectedKey, dirty: false, name: desiredName, quantity: desiredQty, unit: desiredUnit};
        state.itemOverrides = Array.isArray(data.item_overrides) ? data.item_overrides.slice() : state.itemOverrides;
        await loadList(state.data.id, {preserveLocal: true, silent: true, reason: 'override'});
        els.overrideItem.value = selectedKey;
        state.overrideDraft.dirty = false;
        syncOverrideEditor(true);
        setStatus(els.overrideStatus, (desiredName || desiredQty || desiredUnit) ? 'Override saved for this shopping-list item.' : 'Override removed.', 'success');
      } catch (error) {
  ...
  if (els.overrideList) {
    els.overrideList.addEventListener('click', async function (event) {
      ...
      try {
        var data = await fetchJson('/api/shopping-print/item-overrides/delete', {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({list_id: state.data.id, key: row.dataset.overrideKey}),
        });
        if (!data.ok) throw new Error(data.error || 'Could not remove override.');
        state.itemOverrides = Array.isArray(data.item_overrides) ? data.item_overrides.slice() : [];
        if (state.overrideDraft.key === row.dataset.overrideKey) state.overrideDraft = {key: '', dirty: false, name: '', quantity: '', unit: ''};
        await loadList(state.data.id, {preserveLocal: true, silent: true, reason: 'override'});
  ...
  document.querySelectorAll('#sp-width,#sp-margin,#sp-font,#sp-line-gap,#sp-category-gap,#sp-bottom-margin,#sp-density,#sp-threshold,#sp-dpi,#sp-label-type,#sp-show-items,#sp-show-quantities,#sp-show-item-dividers,#sp-show-category-dividers,input[name="sp-category-divider-style"],input[name="sp-item-marker-style"]').forEach(function (input) {
  ...
  els.saveSettings.addEventListener('click', async function () {
    ...
    try {
      ...
      if (!data.ok) throw new Error(data.error || 'Could not save global settings.');
  ...
  els.print.addEventListener('click', async function () {
    ...
    try {
      ...
      if (!data.ok) throw new Error(data.error || 'Printing failed.');
