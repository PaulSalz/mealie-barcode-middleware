...
FONT_CSS = {
    ...
    # Uses OpenDyslexic/Atkinson when installed and a wider Verdana fallback otherwise.
...
def save_theme(db, values: dict[str, str]) -> list[str]:
    ...
    for key, default in THEME_DEFAULTS.items():
        new_val = str(values.get(key, default))
...
def build_theme_css(theme: dict[str, str]) -> str:
    ...
    if font == "dyslexia":
        extra_rules.append("body{letter-spacing:.018em;word-spacing:.045em;line-height:1.55}")
        extra_rules.append("input,select,textarea,button{letter-spacing:.012em}")
    ...
    if epaper:
        ...
        mono_props = [
            "--tblr-primary:#000", "--tblr-primary-rgb:0,0,0",
            "--tblr-body-color:#000", "--tblr-body-bg:#fff",
            f"--tblr-bg-surface:rgb({surface},{surface},{surface})",
            f"--tblr-border-color:rgb({border},{border},{border})",
            f"--tblr-secondary-color:rgb({muted},{muted},{muted})",
        ...
        extra_rules.extend([
            ...
            "body,.page,.page-wrapper{background:#fff!important;color:#000!important}",
